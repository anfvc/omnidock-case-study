...
otto_categories = (
    categories_otto["Category"].dropna().astype(str).tolist()
)  # Converting to strings and ensuring no NaN are part of the filter


def classify_with_openai(
    title: str, description: str
) -> str:  # return type of the function must be a string
    """
    ...
    relevant_categories = [
        category
        for category in otto_categories
        if any(
            word.lower() in category.lower()
    ...
    **Instructions:**
    ...
    response = client.chat.completions.create(
        model="gpt-4o", messages=[{"role": "system", "content": prompt}], max_tokens=20
    )
